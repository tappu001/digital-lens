...
(function (root) {
  ...
  const INTEGRATION_RULES = [
    ...
    { name: 'Meta Pixel', category: 'Advertising', re: /connect\.facebook\.net|fbq\s*\(|facebook\.com\/tr/i, ids: [/fbq\\?['"]init['"]?\\?\s*[,)]\\?\s*['"]?(\\d{10,20})/i] },
    { name: 'TikTok Pixel', category: 'Advertising', re: /analytics\.tiktok\.com|ttq\./i, ids: [/ttq\\?['"]load['"]?\\?\s*[,)]\\?\s*['"]([A-Z0-9]{15,25})/i] },
    { name: 'Snapchat Pixel', category: 'Advertising', re: /sc-static\.net|snaptr\s*\(/i, ids: [/snaptr\\?['"]init['"]?\\?\s*[,)]\\?\s*['"]([a-f0-9-]{20,40})/i] },
    { name: 'Pinterest Tag', category: 'Advertising', re: /s\.pinimg\.com|pintrk\s*\(/i, ids: [/pintrk\\?['"]load['"]?\\?\s*[,)]\\?\s*['"]?(\\d{8,20})/i] },
    ...
    { name: 'Reddit Pixel', category: 'Advertising', re: /redditstatic\.com|rdt\s*\(/i, ids: [/rdt\\?['"]init['"]?\\?\s*[,)]\\?\s*['"]([A-Z0-9_-]{8,40})/i] },
